...
def load_data(file_source, file_name=""):
    ...
    try:
        if source_name.endswith(".parquet"):
            df = pd.read_parquet(file_source, engine="pyarrow")
        else:
...
# -----------------------------
# Sidebar: data source from Google Drive
# -----------------------------
st.sidebar.header("⚙️ إعدادات الداشبورد")

file_url = "https://drive.google.com/uc?export=download&id=1Y0ES8hbdNF_4Rzmskb-cS10jJcfu9IxP"

st.sidebar.caption("مصدر البيانات:")
st.sidebar.code("Google Drive / merged_data.parquet", language="text")

with st.spinner("جاري تحميل البيانات..."):
    df = load_data(file_url, "merged_data.parquet")
...
with tab5:
    ...
    st.markdown("### ملاحظات")
    st.caption("الداشبورد يقرأ البيانات من Google Drive ويطبّق الفلاتر بشكل حي على المؤشرات والرسوم.")
